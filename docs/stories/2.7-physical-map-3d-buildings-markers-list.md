...
- [X] **Task 9: Manual Testing** (AC: All)
  - [X] Test Scenario 1: User views map with 3D buildings at zoom 15+ → Verify buildings render with tilt
  - [X] Test Scenario 2: User sees markers for all Physical somethings → Verify brightness varies by care
  - [X] Test Scenario 3: User clicks marker → Modal shows experiences at that location, sorted by date
  - [X] Test Scenario 4: User clicks [View Details] → Navigates to something detail page
  - [X] Test Scenario 5: Mobile user opens modal → Bottom sheet appears, scrollable
