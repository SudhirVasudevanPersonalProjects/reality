/**
 * /my-reality/somewhere/[somethingId] Page
 *
 * Detail view for a specific something (orb)
 */
...
export default async function SomethingDetailPage({
  ...
  // Parse the orb index from "something1" → 1
  const orbIndexMatch = params.somethingId.match(/^something(\d+)$/)
  const orbIndex = orbIndexMatch ? parseInt(orbIndexMatch[1], 10) - 1 : -1

  // Get the actual orb ID from the index
  const selectedOrbId =
    orbIndex >= 0 && orbIndex < (somethings?.length || 0)
      ? somethings?.[orbIndex]?.id
      : undefined
